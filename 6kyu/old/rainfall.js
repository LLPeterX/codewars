...
Даны две строки - data и data1.
В каждой - города и помесячное кол-во осадков - от января до декабря
Написать ф-ции, аргументами которой город и либо data, либо data1
*/

function getRainData(str) {
  let data = {};
  str.split("\n").forEach(s => {
    let [city, rainData] = s.split(':');
    data[city] = rainData.split(',').map(x => +x.split(' ')[1]);
  });
  return data;
}

function mean(town, strng) {
  let data = getRainData(strng);
  return data[town] ? data[town].reduce((sum, v)=>sum+v,0)/12 : -1;
}
function variance(town, strng) {
  ...
  return data[town].reduce((sum,v) => sum + Math.pow(v-m,2),0)/12;
}
