...
Each emote chain represents the digits of the ASCII/Unicode code for a character, 
e.g. :( ;) is 97, which is the ASCII code for 'a'.
...
function deEmojify(emojiString) {
  ...
  // let chains = emojiString
  //   .split("  ")
  //   .map((emos) => emos.split(" ").map((e) => EMO[e]).join``)
  //   .map(Number);
  return (
    (emojiString &&
